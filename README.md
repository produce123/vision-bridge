...
**实测对比（2026-08-17 · 同一张测试截图 · `analyze_image` · 真实调用豆包）**：

| 档位 | 输出风格 | 字数 | 特点 |
|------|---------|------|------|
| `economy` | 要点式列表 | ~350 | 错误文本 / 堆栈 / 卡片文字关键信息完整保留，最省 |
| `standard` | 结构化叙述 | ~500 | 平衡，含布局与细节说明 |
| `detailed` | 分节详尽 + 总结 | ~800 | 捕捉最细微细节（如"报错行右端被裁切，仅露半个括号"），附整体总结 |

选档建议：快速定位 / 低要求 → `economy`；常规分析 → `standard`；高精度还原 / 复杂图表 → `detailed`。
...
**2026-08-17**（v1.1.0 · 三档识别模式）
- 三档全链路 E2E 通过：economy / standard / detailed 三档真实调用豆包均正常返回。
- 三档对比实测：economy 要点式 ~350 字、standard 结构化 ~500 字、detailed 分节详尽 ~800 字（能捕捉"报错行右端仅露半个括号"等最细微细节）。详见「三档识别模式」。
...
MIT © produce123（完整条款见 [LICENSE](LICENSE)）
